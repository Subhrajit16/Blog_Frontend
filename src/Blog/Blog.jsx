...
function Blog() {
    ...
    async function onSubmit(data) {
        ...
        try {
            setIsLoading(true)
            const resp = axios.post('http://localhost:8080/blog/create', formData, {
                headers: {
    ...
    async function getAllBlogs() {
        ...
        try {
            const resp = await axios.get(`http://localhost:8080/blog/all?page=${currentPage}&limit=${itemsPerPage}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            })
